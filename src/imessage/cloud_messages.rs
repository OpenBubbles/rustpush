...
use crate::cloudkit::{record_identifier, CloudKitSession, CloudKitUploadRequest, DeleteRecordOperation, FetchRecordChangesOperation, FetchRecordOperation, SaveRecordOperation, ALL_ASSETS, NO_ASSETS};
...
use crate::util::{base64_decode, bin_deserialize, bin_serialize, coder_encode_flattened, decode_hex, encode_hex, gzip, plist_to_bin, ungzip, NSAttributedString, NSDictionaryTypedCoder, NSNumber, NSString, StreamTypedCoder};
...
pub struct MessageSummaryInfo {
    ...
    pub ampt: Option<Data>, // am part (full text part of ams)
    pub amc: Option<u32>,
...
pub struct AttachmentMeta {
    ...
    #[serde(rename = "ui")]
    pub user_info: MMCSAttachmentMeta,
    #[serde(rename = "fn")]
    ...
    pub pathc: String, // also transfer name
    #[serde(rename = "mdh")]
    pub md5: String, // first 8 bytes of md5 hash of file
}
...
impl<P: AnisetteProvider> CloudMessagesClient<P> {
    ...
    async fn sync_records<T: CloudKitRecord>(&self, zone: &str, continuation_token: Option<Vec<u8>>) -> Result<(Vec<u8>, HashMap<String, T>), PushError> {
        ...
        for change in &response.change {
            let record = change.record.as_ref().unwrap();
            ...
            results.insert(identifier, item);
        }

        Ok((response.client_change_token().to_vec(), results))
    }
    ...
    pub async fn sync_chats(&self, continuation_token: Option<Vec<u8>>) -> Result<(Vec<u8>, HashMap<String, Chat>), PushError> {
        self.sync_records("chatManateeZone", continuation_token).await
    }

    pub async fn save_chats(&self, chats: HashMap<String, Chat>) -> Result<(), PushError> {
        self.save_records("chatManateeZone", chats).await
    }

    pub async fn sync_messages(&self, continuation_token: Option<Vec<u8>>) -> Result<(Vec<u8>, HashMap<String, Message>), PushError> {
        self.sync_records("messageManateeZone", continuation_token).await
    }

    pub async fn save_messages(&self, messages: HashMap<String, Chat>) -> Result<(), PushError> {
        self.save_records("messageManateeZone", messages).await
    }

    pub async fn sync_attachments(&self, continuation_token: Option<Vec<u8>>) -> Result<(Vec<u8>, HashMap<String, Attachment>), PushError> {
        self.sync_records("attachmentManateeZone", continuation_token).await
    }

    pub async fn save_attachments(&self, attachments: HashMap<String, Attachment>) -> Result<(), PushError> {
    ...
    pub async fn upload_attachments<T: Read + Send + Sync>(&self, zone: &RecordZoneIdentifier, files: Vec<(PreparedPut, T, String)>) -> Result<Vec<cloudkit_proto::Asset>, PushError> {
        let container = self.get_container().await?;
        Ok(container.upload_asset(&CloudKitSession::new(), &zone, files.into_iter().map(|f| CloudKitUploadRequest {
            file: Some(f.1),
            record_id: f.2,
            field: "lqa",
    ...
    pub async fn upload_group_photo<T: Read + Send + Sync>(&self, zone: &RecordZoneIdentifier, files: Vec<(PreparedPut, T, String)>) -> Result<Vec<cloudkit_proto::Asset>, PushError> {
        let container = self.get_container().await?;
        Ok(container.upload_asset(&CloudKitSession::new(), &zone, files.into_iter().map(|f| CloudKitUploadRequest {
            file: Some(f.1),
            record_id: f.2,
